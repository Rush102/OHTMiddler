...
using DDS;
using OHTM.NLog_USE;
//using MirleOHT.類別.工具;                   // Roy+180508
...
namespace MirleOHT.類別.DCPS
{
    ...
    public sealed class ErrorHandler
    {
         ...
         * then terminate.
         **/
        public static bool checkStatus(ReturnCode status, string info)                   // Roy*180621
        {
            if (status != ReturnCode.Ok &&
                status != ReturnCode.NoData)
            {
                eqTool.Fun_Log(eqTool.MyLogKind.GeneralProcess, NLog.LogLevel.Error, null,
                     info + ": " + getErrorName(status));                   // Roy*180621

                //System.Console.ReadLine();                        // Roy-180621
                //System.Environment.Exit(-1);                        // Roy-180621

                return false;                   // Roy+180621
            }
            else
            {
                return true;                   // Roy+180621
            }
        }
         ...
         **/
        public static bool checkHandle(object handle, string info)                   // Roy*180621
        {
            if (handle == null)
            {
                eqTool.Fun_Log(eqTool.MyLogKind.GeneralProcess, NLog.LogLevel.Error, null,
                     info + ": Creation failed: invalid handle");                   // Roy*180621
